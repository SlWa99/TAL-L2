...
class PerceptronTagger(TaggerI):
    ...
    def save_to_json(self, loc, lang="xxx"):
        ...
        with open(loc + "_" + TAGGER_JSONS[lang]["weights"], "w") as fout:
            json.dump(self.model.weights, fout)
        with open(loc + "_" + TAGGER_JSONS[lang]["tagdict"], "w") as fout:
            json.dump(self.tagdict, fout)
        with open(loc + "_" + TAGGER_JSONS[lang]["classes"], "w") as fout:
    ...
    def load_from_json(self, loc = "", lang="xxx"):
        ...
        with open(loc + "_" + TAGGER_JSONS[lang]["weights"]) as fin:
            self.model.weights = json.load(fin)
        with open(loc + "_" + TAGGER_JSONS[lang]["tagdict"]) as fin:
            self.tagdict = json.load(fin)
        with open(loc + "_" + TAGGER_JSONS[lang]["classes"]) as fin:
